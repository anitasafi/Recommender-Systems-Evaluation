...
from Base.Evaluation.metrics import precision, precision_recall_min_denominator, recall, MAP, MAP_MIN_DEN, MRR, HIT_RATE, ndcg, arhr_all_hits, \
    ...
    Diversity_Herfindahl, AveragePopularity, unexpectedness, serendipity
...
class Evaluator(object):
    ...
    def _compute_metrics_on_recommendation_list(self, test_user_batch_array, recommended_items_batch_list, scores_batch, results_dict):
        ...
        for batch_user_index in range(len(recommended_items_batch_list)):
            ...
            # The value of this metric does not depend on cutoff size.
            # Calculating this metric once and then just slicing for the needed
            # cutoff significantly increases performance
            unexp = unexpectedness(
                self.URM_test,
                recommended_items[:self.max_cutoff],
                relevant_items,
            )
            ...
            for cutoff in self.cutoff_list:
                ...
                results_current_cutoff[EvaluatorMetrics.UNEXPECTEDNESS.value] += unexp[:cutoff].mean()
                results_current_cutoff[EvaluatorMetrics.SERENDIPITY.value] += serendipity(unexp[:cutoff], is_relevant_current_cutoff)
...
class EvaluatorHoldout(Evaluator):
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, users_to_evaluate, block_size = None):
        ...
        while user_batch_start < len(users_to_evaluate):
            ...
            if self.test_path is not None:
                list_recommended_items.extend(recommended_items_batch_list)
        ...
        if self.test_path is not None:
            result_data = pd.DataFrame(list_recommended_items)
            result_data.to_csv(f'{self.test_path}/{recommender_object.RECOMMENDER_NAME}.csv')
...
class EvaluatorNegativeItemSample(Evaluator):
    ...
    def __init__(self, URM_test_list, URM_test_negative, cutoff_list, min_ratings_per_user=1, exclude_seen=True,
        ...
        :param ignore_items:
        :param ignore_users:
        :param test_path: Optional path for saving results
        """
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, users_to_evaluate, block_size = None):
        ...
        for test_user in users_to_evaluate:
            ...
            if self.test_path is not None:
                list_recommended_items.append(recommended_items[0])
