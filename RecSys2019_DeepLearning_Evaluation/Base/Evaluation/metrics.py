...
def unexpectedness(URM_test, recommended_items, relevant_items):
    """
    Calculates unexpectedness of the `recommended_items` in relation to
    `relevant_items`, expressed as Normalized Pointwise Mutual Information
    (NPMI).
    Source: https://www.cs.ucc.ie/~dgb/papers/Kaminskas-Bridge-2014.pdf

    :param URM_test: User-Rating Matrix of `scipy.sparse` format
    :param recommeded_items: array-like representing items that were recommended to the user
    :param relevant_items: array-like representing items that are relevant to the user
    """
    n_users = URM_test.shape[0]
    p_ij = URM_test.mean(0).A1
    p_ij = np.multiply(
        *np.meshgrid(
            p_ij[recommended_items],
            p_ij[relevant_items],
            indexing='ij',
            sparse=True,
        )
    ...
    joint_probs = (
        URM_test[:, recommended_items].T.dot(URM_test[:, relevant_items]) / n_users
    ).toarray()
    with np.errstate(divide='ignore', invalid='ignore'):
        return np.nan_to_num(
            np.log2(joint_probs / p_ij) / -np.log2(joint_probs)
        ).mean(1)


def serendipity(unexpectedness, relevance):
    """
    Calculates the serendipity of the recommendations, expressed as the mean of
    the product of recommendation's unexpectedness and relevance.
    
    :param unexpectedness: np.ndarray representing unexpectedness of each item in the reccomendations set
    :param relevance: np.ndarray representing relevance of each item in the reccomendations set. Must be of the same shape as `unexpectedness`.
    """
    assert unexpectedness.shape == relevance.shape, (
        'Unexpectedness and relevance must have identical shapes, got: '
        f'{unexpectedness.shape} {relevance.shape}'
    )
    return np.mean(unexpectedness * relevance)
